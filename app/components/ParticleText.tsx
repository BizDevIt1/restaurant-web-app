...
export default function ParticleText({
  ...
  useEffect(() => {
    ...
    if (!container || !canvas || !probe) return undefined;
    ...
    const sampleText = async () => {
      ...
      const measuredFontSize = parseFloat(computed.fontSize) || 48;
      const resolvedFamily =
        fontFamily === "inherit"
          ? computed.fontFamily || "sans-serif"
          : fontFamily;

      const extraPad = Math.max(24, Math.ceil(scatter * 0.8));
      const boxWidth = Math.max(1, Math.ceil(probeRect.width));
      const boxHeight = Math.max(1, Math.ceil(probeRect.height));

      width = boxWidth + extraPad * 2;
      height = boxHeight + extraPad * 2;
      ...
      const font = `${fontWeight} ${measuredFontSize}px ${resolvedFamily}`;
      ...
      const maxParticles = Math.max(
        600,
        Math.min(3200, Math.floor((width * height) / 100))
      );
  ...
  return (
    <span
      ...
      aria-label={text}
    >
      {/* Invisible probe preserving exact natural inline typography width & height */}
      <span
        ref={textProbeRef}
        className="invisible select-none pointer-events-none opacity-0 inline-block"
        aria-hidden="true"
      >
        {text}
      </span>
