...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-300 pointer-events-none">
      <div
        className={`pointer-events-auto transition-all duration-300 ease-out ${scrolled
            ? "w-[calc(100%-1.5rem)] sm:w-[min(1200px,calc(100%-2rem))] mt-1 sm:mt-1.5 rounded-full bg-[var(--bg-deep)]/40 backdrop-blur-xl border border-[var(--border)] py-2 px-4 sm:px-6 shadow-2xl shadow-black/50"
            : "w-full rounded-none bg-transparent border-b border-transparent mt-0 py-2.5 sm:py-3 px-4 sm:px-6"
          }`}
      >
        <div className="max-w-[1200px] mx-auto flex items-center justify-between gap-3 sm:gap-4">
          {/* Brand Mark */}
          <Link href="/" className="flex items-center gap-1.5 sm:gap-2 shrink-0 group focus:outline-none">
            <Image
              src="/logo.png"
              alt="Omnibites Logo"
              width={48}
            ...
            <div className="flex flex-col">
              <span className="font-display font-extrabold text-xl sm:text-2xl tracking-tight flex items-center leading-none">
                <span className="text-[var(--text-hi)]">Omni</span>
                <span className="text-[#f5a623]">bites</span>
              </span>
              <span className="font-mono text-[9px] sm:text-[10px] tracking-wider uppercase text-[var(--text-faint)] mt-0.5 font-medium">
                Restaurant
              </span>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1 shrink-0">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="px-3.5 py-1.5 text-sm font-medium text-[var(--text-lo)] hover:text-[var(--gold)] transition-colors"
              >
                {link.name}
              </Link>
            ))}
